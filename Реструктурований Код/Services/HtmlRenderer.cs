...
namespace RefactoredCommandSystem.Services
{
    public class HtmlRenderer : IRenderer
    {
        public void RenderDocumentStart(Document doc, StringBuilder output)
        {
            ...
            if (!string.IsNullOrEmpty(doc.Title)) output.AppendLine($"<title>{System.Net.WebUtility.HtmlEncode(doc.Title)}</title>");
            output.AppendLine("</head><body>");
            if (!string.IsNullOrEmpty(doc.Title)) output.AppendLine($"<h1>{System.Net.WebUtility.HtmlEncode(doc.Title)}</h1>");
        }
        public void RenderDocumentEnd(Document doc, StringBuilder output) { output.AppendLine("</body></html>"); }

        public void RenderParagraphStart(Paragraph p, StringBuilder output) { output.Append("<p>"); }
        public void RenderParagraphEnd(Paragraph p, StringBuilder output) { output.AppendLine("</p>"); }

        public void RenderListStart(ListElement list, StringBuilder output)
        {
            output.AppendLine(list.Ordered ? "<ol>" : "<ul>");
        }
